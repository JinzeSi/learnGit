...
private:
    ...
    void prepare(uint64_t max)
    {
        ...
        for (i = 0; i < max; ++i) {
            ...
            if(i==max-1)
                EXPECT(std::string(32537, 't'), store.get(32536));
        }
        //EXPECT(std::string(9898, 't'), store.get(9897));
        phase();
...
public://增加草稿文件  read不能删除当读入完成后，删除后面的文件。
...
int main(int argc, char *argv[])
{
    ...
    PersistenceTest test("/Users/sijinze/Desktop/file/file/data", verbose);
